...
def main():
    ...
    polarparam_to_diff_dict(
        THIS_DIR / 'regression_results.yaml',
        vargen,
        VREG,
        VREF
    )
    ...
    def ddt(time,y,diffslv):  
        #print("time: {} u: {}".format(time, y[diffslv.sim_varorder.index('u')]))
        y[diffslv.sim_varorder.index('u')] = np.random.normal(loc= VREG - VREF, scale=0.1 ) 
        result = diffslv.scipy_ddt(y,time)
        
        return result
    

    sol = solve_ivp(ddt, tiv, ode_converter.scipy_init(init_cond), args=(ode_converter,), method='DOP853', t_eval=t)

    for i, var in enumerate(ode_converter.sim_varorder):
        if('dy' in var or 'du' in var):
            continue
        plt.plot(sol['t'], sol['y'][i], label=var)
    plt.legend(loc='best')
    plt.xlabel('t')
    plt.grid()
    plt.show()
